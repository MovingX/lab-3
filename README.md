...
1.В телеграме в поиске вводим BotFather и создаём бота /newbot вводим обычное и уникальное имя бота, получим ссылку и токен
2.Создаём папку проекта и инициализируем node.js командной npm init -y - создаётся package.json и внём пропишем:
...
```
для того чтобы автоматически активировать нам скрипт
3.Установим пакет для работы с телеграм ботом npm i node-telegram-bot api nodemon
4.Создади файл script.js и приступим к написанию бота. Пример как может выглядеть бот:
